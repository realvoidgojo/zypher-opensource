...
export default function ForecastChart() {
  ...
  if (loading) return (
    <div className="p-20 flex flex-col items-center justify-center min-h-[500px]">
      ...
      <p className="mt-6 text-[#9CA3AF] font-medium text-xs animate-pulse">Loading forecast...</p>
    </div>
  );
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip cursor={{ stroke: '#334155', strokeWidth: 1, strokeDasharray: '3 3' }} contentStyle={{ backgroundColor: '#05080f', borderRadius: '12px', border: '1px solid #1e293b', color: '#fff', fontSize: '12px' }} />
                  <Area type="monotone" dataKey="demand" stroke="#22d3ee" fillOpacity={1} fill="url(#colorDemand)" strokeWidth={2.5} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-[#111827] rounded-2xl p-8 border border-[#1F2937]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2 p-5 bg-[#0B0F14] rounded-xl border border-[#1F2937]">
                <p className="font-semibold text-xs text-[#9CA3AF] uppercase tracking-wider">Consumption Trend</p>
                <p className="text-sm text-slate-300 leading-relaxed">
                  Average consumption is <strong className="text-cyan-400">{insight?.dailyAvg} units/day</strong>. Safety stock of <strong className="text-white">{Math.round(insight?.dailyAvg * 7)} units</strong> recommended.
                </p>
              </div>
              <div className={`p-5 rounded-xl border ${isCritical ? 'bg-rose-500/10 border-rose-500/30' : 'bg-emerald-500/5 border-emerald-500/20'}`}>
                <p className="font-semibold text-xs text-[#9CA3AF] uppercase tracking-wider mb-2">Status</p>
        ...
        <div className="xl:col-span-5 xl:sticky xl:top-8" style={{ height: 'calc(100vh - 4rem)' }}>
          <div className="bg-[#111827] rounded-2xl border border-[#1F2937] shadow-sm flex flex-col h-full overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-6 py-5 border-b border-[#1F2937] shrink-0">
              {Icons.Brain}
              <div>
                <h3 className="font-semibold text-[#F9FAFB] text-sm">Forecast Assistant</h3>
                <p className="text-[10px] text-[#4B5563] font-medium mt-0.5">Powered by AI · Always on</p>
              </div>
              <span className="ml-auto text-[10px] text-[#9CA3AF] font-bold tracking-wider uppercase border border-[#374151] px-2 py-1 rounded-full bg-[#0B0F14]">AI Agent</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4 custom-scrollbar min-h-0">
              {chatMessages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full text-center opacity-60 px-4">
                  {Icons.Sparkle}
                  <p className="text-[#9CA3AF] text-sm font-medium mt-4">How can I help you analyze the forecast?</p>
                  <p className="text-[#6B7280] text-xs mt-1">Select a suggestion below to begin</p>
                </div>
              )}

              {chatMessages.map((m, idx) => (
                <div key={idx} className={`flex ${m.role === 'assistant' ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[85%] p-3.5 rounded-2xl text-[13px] font-medium leading-relaxed shadow-sm ${m.role === 'assistant'
