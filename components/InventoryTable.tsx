    ...
    <div className="bg-[#111827] p-8 lg:p-10 rounded-[2rem] border border-[#1F2937] shadow-sm relative overflow-hidden animate-fade-in-up">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 relative z-10 gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-[#9CA3AF] mt-1">Manage and track your products.</p>
        </div>
        <button
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-[#0B0F14]/80 flex items-center justify-center z-50 p-4 backdrop-blur-md">
          <div className="bg-[#111827] p-8 rounded-[2rem] max-w-md w-full shadow-[0_1px_2px_rgba(0,0,0,0.3)] max-h-[90vh] overflow-y-auto border border-[#1F2937] animate-fade-in-up">
            <h3 className="text-lg font-medium text-[#F9FAFB] mb-1">Add Stock</h3>
            <p className="text-sm font-medium text-[#9CA3AF] mb-6">Configure details for your new inventory item.</p>

            <div className="space-y-5">
              <div className="flex bg-[#111827] p-1 rounded-xl border border-[#1F2937]">
                <button onClick={() => setIsNewWarehouse(false)} className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${!isNewWarehouse ? 'bg-[#3B82F6] text-white shadow-sm border border-[#3B82F6]' : 'text-slate-500 hover:text-slate-300'}`}>Existing Warehouse</button>
                <button onClick={() => setIsNewWarehouse(true)} className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${isNewWarehouse ? 'bg-[#3B82F6] text-white shadow-sm border border-[#3B82F6]' : 'text-slate-500 hover:text-slate-300'}`}>New Warehouse</button>
              </div>
              ...
              {isNewWarehouse ? (
                ...
                <div className="border-l-2 border-indigo-500/50 pl-4 py-2">
                  <select value={selectedWhId} onChange={(e) => setSelectedWhId(e.target.value)} className="w-full p-4 bg-[#111827] border border-[#1F2937] rounded-xl text-white outline-none focus:border-[#3B82F6] transition-all text-sm font-medium appearance-none">
              ...
              <div className="pt-5 border-t border-[#1F2937]">
                <input placeholder="Product Name (e.g. MacBook Pro)" value={prodName} onChange={e => setProdName(e.target.value)} className="w-full p-4 bg-[#111827] border border-[#1F2937] rounded-xl text-white placeholder-slate-500 focus:border-[#3B82F6] outline-none transition-all text-sm font-medium mb-4" />
                <input type="number" placeholder="Initial Quantity" value={initialQty} onChange={e => setInitialQty(e.target.value)} className="w-full p-4 bg-[#111827] border border-[#1F2937] rounded-xl text-white placeholder-slate-500 focus:border-[#3B82F6] outline-none transition-all text-sm font-medium" />
              </div>

              <div className="flex gap-3 pt-6">
                <button onClick={() => setShowAddModal(false)} className="flex-1 py-3 border border-[#1F2937] rounded-xl font-medium text-[#9CA3AF] hover:bg-[#1F2937] transition-all text-sm">Cancel</button>
                <button onClick={handleAddNewEntry} disabled={loading} className="flex-1 py-3 bg-[#3B82F6] hover:bg-[#2563EB] text-white rounded-xl font-semibold transition-all shadow-sm text-sm disabled:opacity-50">
                  {loading ? "Saving..." : "Save"}
                </button>
              </div>
            </div>
              ...
              return (
                <tr key={item.id} className={`bg-[#161b2a] border border-white/5 hover:bg-[#1e2436] transition-colors duration-300 group ${isZero ? 'opacity-50 grayscale-[0.5]' : ''}`}>
                  <td className="p-5 rounded-l-3xl">
                    <p className="font-medium text-[#F9FAFB] text-sm truncate">{item.products?.name}</p>
                  ...
                  <td className="p-5">
                    <div className="flex items-center justify-center gap-1 bg-[#111827] rounded-lg p-1 w-max mx-auto border border-[#1F2937]">
                      <button onClick={() => adjustStock(item.id, item.stock_quantity, false)} disabled={isZero} className="w-8 h-8 rounded-lg flex items-center justify-center bg-[#1F2937] text-white opacity-80 hover:opacity-100 font-bold disabled:opacity-30 transition-all">-</button>
                      <input type="number" min="1" value={amount} onChange={(e) => handleAmountChange(item.id, e.target.value)} className="w-12 text-center font-medium bg-transparent focus:outline-none text-[#F9FAFB] text-sm" />
                      <button onClick={() => adjustStock(item.id, item.stock_quantity, true)} className="w-8 h-8 rounded-lg flex items-center justify-center bg-[#1F2937] text-white opacity-80 hover:opacity-100 font-bold transition-all">+</button>
                    </div>
                  </td>
                  ...
                  <td className="p-5 text-right rounded-r-3xl pr-6">
                    <div className="flex items-center justify-end">
                      {isZero ? (
                        ...
                        <span className="bg-rose-500/10 text-rose-400 px-3 py-1.5 rounded-lg text-xs font-semibold border border-rose-500/20">Critical</span>
                      ) : (
                        <span className="bg-[#1F2937] text-[#10B981] px-3 py-1.5 rounded-lg text-xs font-semibold border border-[#374151] shadow-sm">Optimal</span>
                      )}
                    </div>
                  </td>
                </tr>
