...
export default function AlertsPanel({
  ...
  function processAlerts() {
    ...
    const liveAlerts = shipments
      .map((ship) => {
        const weather = ship.weather_condition || "Clear";
        const isCritical = weather === "Storm" || weather === "Rain";
        return {
          id: ship.id,
          title: isCritical ? `Delay Risk` : `On Track`,
          message: `Shipment "${ship.name}" is passing through a ${weather} zone.`,
          type: isCritical ? "critical" : "safe",
          driverName: ship.driver_name || "Unassigned",
          driverPhone: ship.driver_phone || "",
          truck: ship.truck_number || "—",
        };
      })
  ...
  return (
    <div className="bg-[#111827] p-6 lg:p-8 rounded-xl border border-[#1F2937] h-auto min-h-[300px] lg:h-[500px] flex flex-col shadow-sm">
      <div className="flex justify-between items-center mb-4 border-b border-[#1F2937] pb-4">
        ...
        <button
          onClick={runRealTimeAI}
          className="bg-[#0B0F14] text-[#F9FAFB] px-3 py-1.5 rounded-md text-[12px] font-medium hover:bg-[#1F2937] transition-colors border border-[#374151]"
        >
          Refresh
        </button>
      </div>
          ...
          alerts.map((alert) => (
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div
                  className={`flex items-center gap-1.5 text-[12px] font-medium ${alert.type === "critical" ? "text-[#EF4444]" : "text-[#10B981]"}`}
                >
                  {alert.type === "critical" ? (
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => router.push(`/shipments?track=${alert.id}`)}
                  className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-[12px] font-medium bg-[#1F2937] text-[#F9FAFB] hover:bg-[#374151] transition-all"
                >
                  <Navigation size={12} /> Live Feed
                </button>
                <a
                  href={`tel:${alert.driverPhone}`}
                  className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-[12px] font-medium bg-transparent border border-[#1F2937] text-[#F9FAFB] hover:bg-[#1F2937] transition-all"
                >
                  <Phone size={12} /> Comms
                </a>
              </div>
