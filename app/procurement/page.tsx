...
export default function ProcurementPage() {
  ...
  if (loading) return (
    <div className="p-20 flex flex-col items-center justify-center min-h-[500px]">
      ...
      <p className="mt-6 text-[#9CA3AF] font-medium text-xs animate-pulse">Loading recommendations...</p>
    </div>
  );
      ...
      <div className="bg-[#111827] p-8 lg:p-10 rounded-[2.5rem] border border-[#1F2937] shadow-sm relative overflow-hidden">
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-[#8B5CF6] font-semibold text-xs">Automated Procurement</p>
          </div>
          <h1 className="text-2xl font-semibold text-[#F9FAFB]">Procurement</h1>
          <p className="text-[#9CA3AF] font-medium text-sm mt-1">Automated vendor selection and purchase order generation.</p>
        </div>
      </div>
      ...
      <div className="bg-[#111827] shadow-sm rounded-[3rem] p-10 border border-[#1F2937] text-slate-200">
        ...
        {recommendations.length === 0 ? (
          <div className="text-center py-16 opacity-50">
            ...
            <p className="text-[#10B981] font-semibold text-sm">Stock Levels Healthy</p>
            <p className="text-sm font-medium text-[#9CA3AF] mt-2">All products are currently operating above critical reorder thresholds.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-separate border-spacing-y-3 min-w-[900px]">
              ...
              <tbody>
                {recommendations.map((req) => {
                  ...
                  return (
                    <tr key={req.id} className={`bg-[#111827] border border-[#1F2937] transition-all duration-300 ${isDone ? 'opacity-50 grayscale' : 'hover:bg-[#1F2937] hover:border-[#374151]'}`}>
                      <td className="p-5 rounded-l-3xl">
                        <p className="font-medium text-[#F9FAFB] text-sm">{req.sku}</p>
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-md border ${req.urgency === 'Critical' ? 'bg-rose-500/10 text-rose-400 border-rose-500/20 animate-pulse' : req.urgency === 'High' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-[#3B82F6]/10 text-[#3B82F6] border-[#3B82F6]/20'}`}>
                            {req.urgency}
                          </span>
                      ...
                      <td className="p-5">
                        <p className="text-sm font-medium text-[#D1D5DB]">{req.supplier}</p>
                        <p className="text-xs text-[#9CA3AF] mt-1">Supplier Risk: <span className={req.risk === 'High' ? 'text-rose-400' : req.risk === 'Medium' ? 'text-amber-400' : 'text-[#10B981]'}>{req.risk}</span></p>
                      </td>
                      <td className="p-5 text-center font-medium text-lg text-[#F9FAFB]">{req.qty}</td>
                      <td className="p-5 text-center font-semibold text-[#8B5CF6]">{req.cost}</td>
                      <td className="p-5 text-right rounded-r-3xl pr-6">
                        {isDone ? (
                          <div className="inline-flex items-center gap-2 bg-[#10B981]/10 text-[#10B981] px-4 py-2 rounded-lg border border-[#10B981]/20">
                            {IconCheck} <span className="text-xs font-semibold">Ordered</span>
                          </div>
                        ) : (
                          <button
                          ...
                          >
                            {isWorking ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : "Approve"}
                          </button>
                        )}
                      </td>
                    </tr>
